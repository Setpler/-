...
function toDependencyTypes(val, name) {
  ...
  if (invalidTypes.length > 0) {
    throw new Error(`Flag "${name}" must be a comma separated list of valid dependency types. Received invalid types: "${invalidTypes.join(',')}"`);
  }
  return types;
}
...
function toSpawnOpts(flags) {
  ...
  if (typeof flags.bail !== 'undefined') spawnOpts.bail = boolean(flags.bail, 'bail');
  if (flags.excludeFromGraph) spawnOpts.excludeFromGraph = toDependencyTypes(flags.excludeFromGraph, 'excludeFromGraph');
  // TODO:
...
function toDependency(dependencyString) {
  var _dependencyString$spl = dependencyString.split('@').filter(function (part) {
    return part !== '';
  }),
      name = _dependencyString$spl[0],
      rest = _dependencyString$spl.slice(1);

  var version = rest.join('@');
  if (name.includes('/')) {
    name = '@' + name;
  }
