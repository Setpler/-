...
function runCommandFromCli(args, flags) {
  ...
  if (commandMap.ADD[command]) {
    ...
    } else if (commandMap.CACHE_LIST[subCommand] || typeof subCommand === 'undefined') {
    ...
    if (commandMap.CONFIG_DELETE[subCommand]) {
      ...
      return commands.configCurrent(commands.toConfigCurrentOptions(subCommandArgs, flags));
    }
    ...
    if (commandMap.PROJECT_ADD[subCommand]) {
      ...
      return commands.projectExec(commands.toProjectExecOptions(subCommandArgs, flags));
    ...
    if (commandMap.WORKSPACE_ADD[workspaceCommand]) {
      ...
      return commands.workspaceExec(commands.toWorkspaceExecOptions(workspaceArgs, flags));
    } else if (commandMap.WORKSPACE_LINK[workspaceCommand]) {
      return commands.workspacelink(commands.toWorkspacelinkOptions(workspaceArgs, flags));
      ...
      return commands.workspaceUnlink(commands.toWorkspaceUnlinkOptions(workspaceArgs, flags));
      ...
      return commands.workspaceRun(commands.toWorkspaceRunOptions([subCommand].concat(subCommandArgs), flags));
    }
    ...
    if (commandMap.WORKSPACES_ADD[subCommand]) {
      ...
      return commands.workspacesExec(commands.toWorkspacesExecOptions(subCommandArgs, flags));
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            logger.title(messages.boltVersion(pkg.version), messages.nodeVersion(process.versions.node), { emoji: '⚡️' });

            processes.handleSignals();

            _context.prev = 4;

            _GlobalOptions.globalOptions.setFromFlags(flags);
            _context.next = 8;
